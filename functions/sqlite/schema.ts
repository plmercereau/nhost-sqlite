...
function columnCast(ColumnInfoInternalype: string): ScalarType {
  switch (ColumnInfoInternalype) {
    ...
    case 'number':
    case 'bool':
      return ColumnInfoInternalype as ScalarType
    case 'boolean':
      return 'bool'
    case 'numeric':
      return 'number'
    ...
    default:
      console.log(
        `Unknown SQLite column type: ${ColumnInfoInternalype}. Interpreting as string.`
      )
      return 'string'
  }
}
  ...
  return ddl.statement.flatMap((t: any) => {
    ...
    return t.definition.flatMap((c: any) => {
      if (
        ...
        c.definition.length != 1 ||
        c.definition[0].type != 'constraint' ||
        c.definition[0].variant != 'primary key'
      ) {
        return []
      }
...
export async function getSchema(
  ...
  const filtered: Array<TableInfoInternal> = resultsT.filter(table =>
    includeTable(config, table)
  )
...
export default async (req: Request, res: Response) => {
  ...
  const schema = await getSchema(config, sqlLogger)
