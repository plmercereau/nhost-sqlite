...
import { Config, getConfig } from './_config'
import { connect, SqlLogger } from './_db'
import {
...
function extractRawTableName(tableName: TableName): string {
  if (tableName.length === 1) return tableName[0]
...
function json_object(
  relationships: Array<TableRelationships>,
  fields: Fields,
  table: TableName
): string {
  const result = omap(fields, (fieldName, field) => {
    switch (field.type) {
      ...
      case 'relationship':
        const tableRelationships = relationships.find(tr =>
          tableNameEquals(tr.source_table)(table)
        )
        if (tableRelationships === undefined) {
          throw new Error(
            `Couldn't find table relationships for table ${table}`
          )
        }
        const rel = tableRelationships.relationships[field.relationship]
        if (rel === undefined) {
          throw new Error(
            `Couldn't find relationship ${field.relationship} for field ${fieldName} on table ${table}`
          )
        }
  ...
  switch (expression.type) {
    ...
    case 'and':
      const aAnd = expression.expressions.flatMap(x =>
        where_clause(relationships, x, tableName)
      )
    ...
    case 'binary_op':
      ...
      return expression.column.path.length < 1
        ? `${escapeIdentifier(expression.column.name)} ${bop} ${bop_val(
            expression.value,
            tableName
          )}`
        : exists(
            ...
            expression.column,
            tableName,
            `${bop} ${bop_val(expression.value, tableName)}`
          )
    case 'binary_arr_op':
...
function exists(
  ...
  return tag(
    'exists',
    `EXISTS (SELECT 1 ${f} WHERE ${[
      ...r.j,
      `${last(r.f).as}.${escapeIdentifier(c.name)} ${o}`
    ].join(' AND ')})`
  )
}
...
function join_path(
  ...
  path: Array<string>,
  level: number
): { f: Array<{ from: string; as: string }>; j: string[] } {
  const r = find_table_relationship(ts, table)
  if (path.length < 1) {
    return { f: [], j: [] }
  } else if (r === null) {
    throw new Error(
      `Couldn't find relationship ${ts}, ${table.join(
        '.'
    ...
    const m = omap(
      ...
      (sourceColumnName, targetColumnName) =>
        `${depthQualifyIdentifier(
          level - 1,
          extractRawTableName(table)
        )}.${escapeIdentifier(sourceColumnName)} = ${depthQualifyIdentifier(
          level,
          extractRawTableName(x.target_table)
        )}.${escapeIdentifier(targetColumnName)}`
    ).join(' AND ')
    return {
      f: [
        {
          from: escapeTableName(x.target_table),
          as: depthQualifyIdentifier(level, extractRawTableName(x.target_table))
        },
        ...n.f
      ],
      j: [m, ...n.j]
 ...
 */
function find_table_relationship(
  ts: Array<TableRelationships>,
  t: TableName
...
function aggregates_query(
  ...
  if (isEmptyObject(aggregates)) {
    ...
    return [
      `'aggregates', (SELECT JSON_OBJECT(${aggregate_pairs}) FROM (SELECT * from ${escapeTableName(
        table
      )} ${innerFromClauses}))`
    ]
  }
...
function array_relationship(
  ...
  const innerFromClauses = `${where(ts, wWhere, wJoin, table)} ${order(
    wOrder
  )} ${limit(wLimit)} ${offset(wOffset)}`
  const aggregateSelect = aggregates_query(table, aggregates, innerFromClauses)
  const fieldSelect = isEmptyObject(fields)
    ? []
    : [`'rows', JSON_GROUP_ARRAY(j)`]
    ...
    : (() => {
        ...
        if (wOrder === null || wOrder.elements.length < 1) {
          return `FROM ( SELECT ${json_object(
            ...
            fields,
            table
          )} AS j FROM ${escapeTableName(table)} ${innerFromClauses})`
        } else {
          const innerSelect = `SELECT * FROM ${escapeTableName(
            table
          )} ${innerFromClauses}`
          return `FROM (SELECT ${json_object(
            ts,
            fields,
            table
          )} AS j FROM (${innerSelect}) AS ${table})`
        }
      })()

  return tag(
    'array_relationship',
    `(SELECT JSON_OBJECT(${[...fieldSelect, ...aggregateSelect].join(
      ', '
    )}) ${fieldFrom})`
  )
}
...
function object_relationship(
  ...
  return tag(
    'object_relationship',
    `(SELECT JSON_OBJECT('rows', JSON_ARRAY(${json_object(
      ts,
      fields,
      table
    )})) AS j FROM ${innerFrom})`
...
function relationship(
  ...
  const wJoin = omap(
    r.column_mapping,
    (k, v) =>
      `${escapeTableName(table)}.${escapeIdentifier(k)} = ${escapeTableName(
        r.target_table
      )}.${escapeIdentifier(v)}`
  )
...
function bop_val(v: ComparisonValue, t: TableName): string {
  switch (v.type) {
    case 'column':
      return tag('bop_val', bop_col(v.column, t))
    case 'scalar':
...
function order(orderBy: OrderBy | null): string {
  ...
  const result = orderBy.elements
    .map(orderByElement => {
      ...
      return `${orderByElement.target.column} ${orderDirection(
...
function where(
  ...
  const whereClause =
    whereExpression !== null ? [where_clause(ts, whereExpression, t)] : []
...
export default async (req: Request, res: Response) => {
  ...
  const result = await queryData(config, sqlLogger, req.body)
